		  ...
		  size_t dma_entry_size)
{
	...
	map->state = kmalloc(sizeof(*map->state), GFP_KERNEL);
	if (!map->state)
		return -ENOMEM;

	dma_iova_init(dev, map->state);
	...
	if (dma_can_use_iova(map->state)) {
		dma_addr = dma_iova_alloc(dev, map->state, 0,
			   ...
			   struct pci_p2pdma_map_state *p2pdma_state)
{
	...
	if (dma_can_use_iova(state)) {
		...
		if (ret)
			return DMA_MAPPING_ERROR;
		dma_iova_sync(dev, state, offset, map->dma_entry_size, 0);
...
#undef HMM_PFN_VALID_DMA
	...
	else if (dma_can_use_iova(state))
