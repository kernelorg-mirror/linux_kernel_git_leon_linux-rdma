 ...
 */
dma_addr_t hmm_dma_map_pfn(struct dma_iova_state *state, unsigned long *pfn,
			   size_t offset, dma_addr_t addr)
{
	dma_addr_t dma_addr;
	int ret;
	...
	if (dma_can_use_iova(state)) {
		ret = dma_link_range(state, hmm_pfn_to_phys(*pfn), offset,
				     PAGE_SIZE, 0);
		if (ret)
	...
	dma_addr = dma_map_page(state->dev, hmm_pfn_to_page(*pfn), 0, PAGE_SIZE,
				state->dir);
out:
 ...
 * @addr: DMA address in case it is not IOVA path
 */
void hmm_dma_unmap_pfn(struct dma_iova_state *state, unsigned long *pfn,
		       size_t offset, dma_addr_t addr)
{
	*pfn &= ~HMM_PFN_DMA_MAPPED;
	if (dma_can_use_iova(state)) {
		WARN_ON_ONCE(addr != DMA_MAPPING_ERROR);
		dma_unlink_range(state, offset, PAGE_SIZE, 0);
		return;
	}
