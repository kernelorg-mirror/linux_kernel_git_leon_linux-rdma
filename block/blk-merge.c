...
static void blk_map_iter_next_bio(struct request *req,
		struct req_iterator *iter, struct bio_vec *prev)
{
	struct bio_vec next;
	...
	/*
	 * Check if the first bvec in the new bio can be merged into the last
	 * one of the previous bio.  All other merging is done when adding
	 * data to the bio.
	 */
	next = mp_bvec_iter_bvec(iter->bio->bi_io_vec, iter->iter);
	if (prev->bv_len + next.bv_len <= queue_max_segment_size(req->q) &&
	    biovec_phys_mergeable(req->q, prev, &next)) {
		prev->bv_len += next.bv_len;
		...
		struct req_iterator *iter, struct phys_vec *vec)
{
	...
	if (bv.bv_offset + bv.bv_len > PAGE_SIZE) {
		bv.bv_len = get_max_segment_size(&req->q->limits, vec->paddr,
				bv.bv_len);
	}
	bio_advance_iter_single(iter->bio, &iter->iter, bv.bv_len);

	while (!iter->iter.bi_size && iter->bio)
		blk_map_iter_next_bio(req, iter, &bv);
		...
		struct dma_iova_state *state, struct blk_dma_vec *map)
{
	...
	if (dma_can_use_iova(state)) {
		...
		map->len = total_len;
		map->addr = dma_alloc_iova(state, vec.paddr, total_len);
		if (dma_mapping_error(state->dev, map->addr))
			return BLK_STS_RESOURCE;

		do {
			error = dma_link_next_range(state, vec.paddr, vec.len,
					0);
			if (error) {
				/*
