			 ...
			 size_t size, unsigned long attrs)
{
	...
	return iommu_dma_link_range(state, phys, state->range_size, size, attrs);
}
			     ...
			     dma_addr_t dma_offset)
{
	...
	phys_addr_t phys = hmm_pfn_to_phys(*pfn);
	...
	ret = iommu_dma_link_range(state, phys, dma_offset, PAGE_SIZE, 0);
	if (ret)
		return DMA_MAPPING_ERROR;

	return state->addr + dma_offset;
}
			 ...
			 dma_addr_t dma_offset)
{
	...
	iommu_dma_unlink_range(state, dma_offset, PAGE_SIZE, 0);
}
