		...
		size_t offset, size_t size, unsigned long attrs)
{
	...
	WARN_ON_ONCE(iova_start_pad && state->range_size > 0);

	if (dev_use_swiotlb(state->dev, size, state->dir))
		return -EREMOTEIO;

	if (is_pci_p2pdma_page(page)) {
		enum pci_p2pdma_map_type map =
			pci_p2pdma_map_type(page->pgmap, state->dev);
...
void iommu_dma_unlink_range(struct dma_iova_state *state, size_t offset,
		size_t size, unsigned long attrs)
{
	...
	if (!(attrs & DMA_ATTR_SKIP_CPU_SYNC) && !coherent)
		iommu_sync_dma_for_cpu(domain, state->addr + offset, size,
				       state->dir);
	...
	unmapped = iommu_unmap_fast(domain, state->addr + offset, size,
				    &iotlb_gather);
	WARN_ON(unmapped != size);
	state->range_size -= unmapped;

	iommu_iotlb_sync(domain, &iotlb_gather);
}

void iommu_dma_destroy_iova(struct dma_iova_state *state, dma_addr_t dma_addr,
	    size_t size)
{
	struct iommu_domain *domain = iommu_get_dma_domain(state->dev);
	struct iommu_dma_cookie *cookie = domain->iova_cookie;
