		...
		dma_addr_t dma_offset, size_t size, unsigned long attrs)
{
	...
	dma_addr_t addr = state->addr + dma_offset;
		...
		size_t size, unsigned long attrs)
{
	...
	if (!(attrs & DMA_ATTR_SKIP_CPU_SYNC) && !coherent)
		iommu_sync_dma_for_cpu(domain, state->addr + dma_offset, size,
				       state->dir);

	iommu_iotlb_gather_init(&iotlb_gather);

	size = iova_align(iovad, size);
	unmapped = iommu_unmap_fast(domain, state->addr + dma_offset, size,
				    &iotlb_gather);
	WARN_ON(unmapped != size);
