		...
		size_t iova_start_pad)
{
	...
	return __dma_iova_link(dev, addr - iova_start_pad,
		...
		size_t size, enum dma_data_direction dir, unsigned long attrs)
{
	...
	if (iova_start_pad) {
		size_t bounce_len = min(size, PAGE_SIZE - iova_start_pad);
...
out_unmap:
	iommu_unmap(domain, state->addr + offset - iova_start_pad,
			mapped + iova_start_pad);
	return error;
}
		...
		unsigned long attrs)
{
	...
	do {
		...
		if (WARN_ON(!phys))
			continue;
		len = min_t(size_t, end - addr, PAGE_SIZE - iova_start_pad);
