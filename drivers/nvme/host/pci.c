...
static void nvme_free_descriptors(struct nvme_dev *dev, struct request *req)
{
	...
	for (i = 0; i < iod->nr_descriptors; i++) {
		...
		dma_pool_free(nvme_dma_pool(dev, iod), prp_list, dma_addr);
...
static void nvme_unmap_data(struct nvme_dev *dev, struct request *req)
{
	...
	if (dma_can_use_iova(&iod->dma_state)) {
		...
		dma_destroy_iova(&iod->dma_state,
		...
		struct request *req, struct nvme_rw_command *cmnd)
{
	...
	/*
	 * PRP1 always points to to start the start of the DMA transfers.
	 *
	...
	for (;;) {
		 ...
		 * accomodate for that move the last actual entry to the new
		 * descriptor.
		 */
		if (i == NVME_CTRL_PAGE_SIZE >> 3) {
