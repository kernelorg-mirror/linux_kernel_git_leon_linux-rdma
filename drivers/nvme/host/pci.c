...
static void nvme_unmap_data(struct nvme_dev *dev, struct request *req)
{
	...
	switch (blk_rq_dma_unmap(req, dev->dev, &iod->dma_state)) {
	case BLK_DMA_UNMAP_IOVA:
		dma_iova_destroy(dev->dev, &iod->dma_state, rq_dma_dir(req), 0);
		break;
	case BLK_DMA_UNMAP_SINGLE:
		if (iod->cmd.common.flags & NVME_CMD_SGL_METABUF)
			nvme_free_sgls(dev, req);
		else
			nvme_free_prps(dev, req);
		break;
	case BLK_DMA_UNMAP_NONE:
		break;
	}
