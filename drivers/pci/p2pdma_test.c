...
static const struct hmat_route_case hmat_route_cases[] = {
	...
	{
		.desc = "cpu_fallback_without_hmat",
		.cpu_p2pdma = true,
		.client_pxm = 11,
		.provider_pxm = 22,
		.lookup_ret = -ENOENT,
	...
	{
		.desc = "whitelist_fallback_without_hmat",
		.whitelist = true,
		.client_pxm = 11,
		.provider_pxm = 22,
		.lookup_ret = -ENOENT,
...
static void pci_p2pdma_hmat_route_test(struct kunit *test)
{
	...
	if (test_case->whitelist) {
		ctx.provider->vendor = PCI_VENDOR_ID_GOOGLE;
		ctx.client->vendor = PCI_VENDOR_ID_GOOGLE;
	}
	test->priv = &ctx;
