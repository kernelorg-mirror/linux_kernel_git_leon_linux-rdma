...
static int mlx5vf_add_migration_pages(struct page **page_list,
				      unsigned int npages)
{
	unsigned int filled = 0, done = 0;
	int i;

	page_list = kvcalloc(npages, sizeof(*page_list), GFP_KERNEL_ACCOUNT);
	if (!page_list)
		return -ENOMEM;
			 ...
			 enum dma_data_direction dma_dir)
{
	...
	buf->migf = migf;
	if (npages) {
		ret = mlx5vf_add_migration_pages(buf->page_list, npages);
		if (ret)
			goto end;
					  ...
					  u64 rq_size)
{
	...
	err = mlx5vf_add_migration_pages(recv_buf->page_list, npages);
	if (err)
		return err;
