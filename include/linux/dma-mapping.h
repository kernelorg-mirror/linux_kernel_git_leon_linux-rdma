 ...
 * @state: IOVA state
 *
 * Return %true if the transaction should used dma_iova*() and related
 * functions, otherwise it should use dma_map_page() and related.
 */
...
static inline int dma_iova_sync(struct device *dev, size_t offset,
		struct dma_iova_state *state, size_t size, int ret)
{
	return -EOPNOTSUPP;
}
